...
function handleAdoptionFormSubmit(event) {
  ...
  saveAdoptionDetails(petType, breed, name, email, phone);
...
function saveAdoptionDetails(petType, breed, name, email, phone) {
  ...
  const request = window.indexedDB.open("AdoptionDB", 1);

  request.onupgradeneeded = function (event) {
    const db = event.target.result;
    const objectStore = db.createObjectStore("adoptionDetails", {
      keyPath: "id",
      autoIncrement: true,
    });
  };
  ...
  request.onsuccess = function (event) {
    const db = event.target.result;
    const transaction = db.transaction("adoptionDetails", "readwrite");
    const objectStore = transaction.objectStore("adoptionDetails");
    const addRequest = objectStore.add(adoptionDetails);
    ...
    addRequest.onerror = function () {
      console.error("Error saving adoption details:", addRequest.error);
    };
  ...
  request.onerror = function (event) {
    console.error("Error opening AdoptionDB:", event.target.error);
  };
}
